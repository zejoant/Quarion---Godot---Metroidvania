...
@export_enum("SkyfishAura", "SpiderGauntlets", "SwiftwindAmulet", "PegasusBoots", "FreezewakeCharm") var type = "SkyfishAura"
...
func play_animation(body):
	...
	cam.zoom_camera(1, 0)
	cam.rotation = 0
	#print(body.get_node("Sprite2D").material.get_shader_parameter("palette_choice"))
	#if body.get_node("Sprite2D").material.get_shader_parameter("palette_choice"):
	body.get_node("Sprite2D").material.set_shader_parameter("palette_choice", body.get_node("Sprite2D").material.get_shader_parameter("palette_choice")+1)
	body.currentPalette += 1
