...
@export_enum("green", "red", "blue") var type = "green" 
...
func _ready():
	for pos in get_node("/root/World").get_room_state():
		if Vector2i(position/8) == Vector2i(pos): #check if boss has been defeated
			queue_free()
	if type == "red":
		$KeySprite.region_rect.position = Vector2(16, 112)
	elif type == "blue":
...
func collect():
	...
	if type == "green":
		...
		player.blue_key_state = "collected"
		cam.add_collected_item(cam.CollectedItem.BLUE_KEY)
